<footer class="footer">
    <div class="footer-columnas">
        <div class="footerIMGizquierda">
            <img src="{{ asset('assets/img/footer/1.png') }}" alt="Chun Li SF6" class="img-fluid">
        </div>
        <div>
            <div class="footer-parrafo"> 
                <p>Todos los derechos reservados. Creadora: HimoutoKwai.</p>
            </div>
            <div class="social-media">
                <a href="mailto:your-email@gmail.com" target="_blank" class="social-icon"><i class="fab fa-google"></i> Gmail</a>
                <a href="https://wa.me/yourphonenumber" target="_blank" class="social-icon"><i class="fab fa-whatsapp"></i> WhatsApp</a>
                <a href="https://www.facebook.com" target="_blank" class="social-icon"><i class="fab fa-facebook"></i> Facebook </a>
                <a href="https://www.instagram.com" target="_blank" class="social-icon"><i class="fab fa-instagram"></i> Instagram</a>
                <a href="https://discord.com" target="_blank" class="social-icon"><i class="fab fa-discord"></i> Discord</a>
                <a href="https://www.twitter.com" target="_blank" class="social-icon"><i class="fab fa-twitter"></i> Twitter</a>
                <a href="https://github.com" target="_blank" class="social-icon"><i class="fab fa-github"></i> GitHub</a>
            </div>
        </div>
        <div class="footerIMGizquierda">
            <img src="{{ asset('assets/img/footer/2.png') }}" alt="Chun Li SFV" class="img-fluid">
        </div>
    </div>
</footer>
